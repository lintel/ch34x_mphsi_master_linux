 ...
 * V1.2 - add supports for i2c communication of long packets, use workqueue to
 * implement irq setting operation,
 *      - support more spi clock frequency
 * V1.3 - add supports for gpio level triggered interrupt, add mutex in
 * ch347_spi_transfer_one_message V1.4 - fix the big-endian CPU compatibility
 * issue during SPI configuration
 *      - fix the problem that obtaining GPIO status will change GPIO
 * configuration
 */
...
static const struct usb_device_id ch34x_usb_ids[] = {
    ...
    {USB_DEVICE_INTERFACE_NUMBER(0x1a86, 0x55db,
                                 0x02)}, /* CH347T Mode1 SPI+IIC+UART */
    {}                                   /* Terminating entry */
};
      ...
      for (i = 0; i < CH347T_MPHSI_GPIOS; i++) {
        ...
        if (ch34x_dev->firmver >= 0x0341) {
          ...
          DEV_DBG(CH34X_USBDEV, "%s %s irq=%d %s",
                  ch347cfg->mode == GPIO_MODE_IN ? "input " : "output",
...
static void ch34x_usb_complete_intr_urb(struct urb *urb) {
  ...
  if (!urb->status) {
    ...
    if (ch34x_dev->chiptype != CHIP_CH341) {
      ...
      io_status =
          (ch34x_dev->intrin_buf[1] & 0x0F) << 8 | ch34x_dev->intrin_buf[2];
...
static int ch34x_usb_probe(struct usb_interface *intf,
  ...
  for (i = 0; i < iface_desc->desc.bNumEndpoints; i++) {
    ...
    DEV_DBG(CH34X_USBDEV, "    ->endpoint=%d type=%d dir=%d addr=%0x", i,
            usb_endpoint_type(endpoint), usb_endpoint_dir_in(endpoint),
