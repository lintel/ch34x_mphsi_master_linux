...
MODULE_PARM_DESC(gpio_base_num,
                 "GPIO master base number (if negative, dynamic allocation)");
...
static void ch347_irq_enable_control(struct irq_data *data, bool enable) {
  ...
  if (!data || !(ch34x_dev = irq_data_get_irq_chip_data(data)))
    return;
 ...
 * The function return true if success, others if fail.
 */
static int ch347gpio_get(struct ch34x_device *ch34x_dev, u8 *cfg_data, u8 *idir,
                         u8 *idata) {
  int j;
 ...
 * @idataout: gpio output bits, bits0-7 on gpio0-7, if gpio direction is output,
 * 1 on high, 0 on low
 *
 * The function return true if success, others if fail.
 */
static int ch347gpio_set(struct ch34x_device *ch34x_dev, u8 ienable, u8 idirout,
                         u8 idataout) {
  u8 cfg_data[CH347_GPIO_CNT] = {0};
  int i;
...
static int ch34x_mphsi_gpio_get(struct gpio_chip *chip, unsigned offset) {
  ...
  dev_dbg(CH34X_USBDEV, "offset=%u, gpio%d, value=%d", offset, gpioindex,
          value);
...
static void irq_set_work(struct work_struct *work) {
  struct ch34x_device *ch34x_dev =
      container_of(to_delayed_work(work), struct ch34x_device, work);
  ...
  for (irq = 0; irq < ch34x_dev->irq_num; irq++) {
    gpioindex = ch34x_dev->gpio_pins[irq]->gpioindex;
    ch347_irq_control(ch34x_dev, gpioindex, ch34x_dev->irq_enabled[irq],
...
int ch34x_mphsi_gpio_probe(struct ch34x_device *ch34x_dev) {
  ...
  dev_info(CH34X_USBDEV, "registered GPIOs from %d to %d", gpio->base,
