			    ...
			    void *txbuf, u32 rxlen, void *rxbuf)
{
	...
	if (rxlen) {
		len = ch34x_usb_transfer(
		    ch34x_dev, i,
		    (rxlen + (CH341_USB_MAX_BULK_SIZE - 1)) /
			CH341_USB_MAX_BULK_SIZE * CH341_USB_MAX_BULK_SIZE);
		if (len != rxlen) {
			retval = -EPROTO;
			goto error;
		}
			    ...
			    void *txbuf, u32 rxlen, void *rxbuf)
{
	...
	if (rxlen) {
		len = ch34x_usb_transfer(ch34x_dev, i,
					 (rxlen + (CH347_USB_BULK_EPSIZE - 1)) /
					     CH347_USB_BULK_EPSIZE *
		...
		len -= ackbits;
		memmove(ch34x_dev->bulkin_buf, ch34x_dev->bulkin_buf + ackbits,
			len);
		memcpy(rxbuf, ch34x_dev->bulkin_buf, rxlen);
	} else {
...
static int ch34x_i2c_xfer(struct i2c_adapter *adapter, struct i2c_msg *msgs,
			  int num)
{
	...
	if (num == 1) {
		...
		if (msgs[0].flags & I2C_M_RD) {
			...
			if (ch34x_dev->chiptype == CHIP_CH341)
				retval =
				    ch341_i2c_stream(ch34x_dev, 0x01, txbuf,
						     msgs[0].len, msgs[0].buf);
			else
				retval =
				    ch347_i2c_stream(ch34x_dev, 0x01, txbuf,
						     msgs[0].len, msgs[0].buf);
			if (retval < 0)
				goto exit;
		} else {
			...
			if (ch34x_dev->chiptype == CHIP_CH341)
				retval = ch341_i2c_stream(
				    ch34x_dev, msgs[0].len + 1, txbuf, 0, NULL);
			else
...
int ch34x_mphsi_i2c_probe(struct ch34x_device *ch34x_dev)
{
	...
	if ((ch34x_dev->firmver >= 0x0341) ||
		...
		if (retval < 0) {
			dev_err(&ch34x_dev->adapter.dev,
				"set i2c stretch failed\n");
			goto error;
		}
	}

	dev_info(CH34X_USBDEV, "I2C master connected to I2C bus %d",
