...
MODULE_PARM_DESC(spi_bus_num,
		 "SPI master bus number (if negative, dynamic allocation)");
...
static void ch341_spi_update_io_data(struct ch34x_device *ch34x_dev)
{
	...
	ch34x_dev->bulkout_buf[1] =
	    CH341_CMD_UIO_STM_DIR | ch34x_dev->gpio_mask;
...
static void ch341_spi_set_cs(struct spi_device *spi, bool active)
{
	struct ch34x_device *ch34x_dev =
	    ch34x_spi_maser_to_dev(spi_device_get_master(spi));

	if (spi->mode & SPI_NO_CS)
		return;

	if (spi_device_get_chip_select(spi, 0) > CH341_SPI_MAX_NUM_DEVICES) {
		dev_err(CH34X_USBDEV, "invalid CS value %d, 0~%d are available",
			spi_device_get_chip_select(spi, 0),
			CH341_SPI_MAX_NUM_DEVICES - 1);
	}

	if (active)
		ch34x_dev->gpio_io_data &=
		    ~(1 << spi_device_get_chip_select(spi, 0));
	else
		ch34x_dev->gpio_io_data |=
		    (1 << spi_device_get_chip_select(spi, 0));
...
static bool ch347spi_get_hwcfg(struct ch34x_device *ch34x_dev,
			       struct ch34x_stream_hw_config *streamcfg)
{
...
static void hwcfg_cpu_to_le(struct ch34x_stream_hw_config *hwcfg)
{
	hwcfg->spi_rw_interval = __cpu_to_le16(hwcfg->spi_rw_interval);
	hwcfg->spi_init.direction = __cpu_to_le16(hwcfg->spi_init.direction);
	hwcfg->spi_init.mode = __cpu_to_le16(hwcfg->spi_init.mode);
	hwcfg->spi_init.datasize = __cpu_to_le16(hwcfg->spi_init.datasize);
	hwcfg->spi_init.clock_pol = __cpu_to_le16(hwcfg->spi_init.clock_pol);
	hwcfg->spi_init.clock_phase =
	    __cpu_to_le16(hwcfg->spi_init.clock_phase);
	hwcfg->spi_init.nss = __cpu_to_le16(hwcfg->spi_init.nss);
	hwcfg->spi_init.baudrate_scale =
	    __cpu_to_le16(hwcfg->spi_init.baudrate_scale);
	hwcfg->spi_init.first_bit = __cpu_to_le16(hwcfg->spi_init.first_bit);
	hwcfg->spi_init.crc_poly = __cpu_to_le16(hwcfg->spi_init.crc_poly);
}
 ...
 */
static bool spicfg_to_hwcfg(struct ch34x_spi_config *spicfg,
			    struct ch34x_stream_hw_config *hwcfg)
{
	...
	spicfg->byte_order &= 0x01;
	hwcfg->spi_init.first_bit = spicfg->byte_order ? 0x00 : 0x80;

	switch (spicfg->mode) {
	case 0:
		hwcfg->spi_init.clock_phase = SPI_CPHA_1Edge;
		hwcfg->spi_init.clock_pol = SPI_CPOL_Low;
		break;
	case 1:
		hwcfg->spi_init.clock_phase = SPI_CPHA_2Edge;
		hwcfg->spi_init.clock_pol = SPI_CPOL_Low;
		break;
	case 2:
		hwcfg->spi_init.clock_phase = SPI_CPHA_1Edge;
		hwcfg->spi_init.clock_pol = SPI_CPOL_High;
		break;
	case 3:
		hwcfg->spi_init.clock_phase = SPI_CPHA_2Edge;
		hwcfg->spi_init.clock_pol = SPI_CPOL_High;
	...
	hwcfg->spi_init.baudrate_scale = spicfg->clock * 8;
	hwcfg->spi_out_def = spicfg->out_def;
	hwcfg->spi_rw_interval = spicfg->rw_interval;
...
{
	u8 *io = ch34x_dev->bulkout_buf;
	struct ch34x_stream_hw_config hwcfg = {{0}, 0};
	int len, i;
	...
	len = sizeof(struct ch34x_stream_hw_config);
	io[i++] = (u8)(len & 0xFF);
	io[i++] = (u8)((len >> 8) & 0xFF);
		...
		if (ch34x_usb_transfer(ch34x_dev, 0, len) == len) {
			...
			if (ch34x_dev->bulkin_buf[USB20_CMD_HEADER] == 0) {
				memcpy(&ch34x_dev->spicfg, spicfg,
				       sizeof(struct ch34x_spi_config));
				memcpy(&ch34x_dev->hwcfg, &hwcfg,
				       sizeof(struct ch34x_stream_hw_config));
				return true;
			}
		}
	}
 ...
 * @ienable_cs: low 8 bits: CS0, high 8 bits: CS1, byte value -> 1: set CS, 0:
 * ignore CS setting
 * @ics: low 8 bits: CS0, high 8 bits: CS1, CS output, byte value -> 1: set CS,
 * 0: cancel CS
 * @iauto_de_cs: low 16 bits: CS0, high 16 bits: CS1, automatically undo the CS
 * after operation completed
 * @iactive_delay: low 16 bits: CS0, high 16 bits: CS1, delay time of read and
 * write operation after setting CS, unit: us
 * @ideactive_delay: low 16 bits: CS0, high 16 bits: CS1,, delay time of read
 * and write operation after canceling CS, unit: us
 *
 */
...
static bool ch347spi_change_cs(struct ch34x_device *ch34x_dev, u8 istatus)
{
	...
	iauto_de_cs = ch34x_dev->spicfg.auto_de_cs;
	iactive_delay = ch34x_dev->spicfg.active_delay;
	ideactive_delay = ch34x_dev->spicfg.deactive_delay;
	ics = istatus;

	/* CS1 selected */
	if (ch34x_dev->spicfg.cs & 0x8000) {
		ienable_cs = 0x0100;
		ics = (ics << 8) & 0xFF00;
...
static bool ch347_spi_set_cs(struct spi_device *spi, bool active)
{
	...
	if (spi_device_get_chip_select(spi, 0) > CH347_SPI_MAX_NUM_DEVICES) {
		dev_err(CH34X_USBDEV, "invalid CS value %d, 0~%d are available",
			spi_device_get_chip_select(spi, 0),
			CH341_SPI_MAX_NUM_DEVICES - 1);
	}

	if (spi_device_get_chip_select(spi, 0) == 0)
		ch34x_dev->spicfg.cs = 0x80;
	else
		ch34x_dev->spicfg.cs = 0x80 << 8;
 ...
 *
 * CPOL=0, CPHA=0   data must be stable while clock is high, can be changed
 * while clock is low mode0            data sampled on raising clock edge
 *
 * CPOL=0, CPHA=1   data must be stable while clock is low, can be changed while
 * clock is high mode1            data sampled on falling clock edge
 *
 * CPOL=1, CPHA=0   data must be stable while clock is low, can be changed while
 * clock is high mode2            data sampled on falling clock edge
 *
 * CPOL=1, CPHA=1   data must be stable while clock is high, can be changed
 * while clock is low mode3            data sampled on raising clock edge
 */
			     ...
			     int len)
{
	...
	for (b = 0; b < len; b++) {
		...
		result = ch34x_usb_transfer(ch34x_dev, k, 8);
		if (result < 0)
			return result;
		...
		for (i = 0; i < 8; i++) {
			byte = byte << 1;
			byte = byte |
			       ((ch34x_dev->bulkin_buf[i] & MISO_BIT) ? 1 : 0);
		}
			    ...
			    int len)
{
	...
	while (len) {
		...
		/* fill output buffer with command and output data, master
		 * expects lsb first
		 */
		...
		if (rx) {
			/* fill input data with input buffer, master delivers
			 * lsb first */
			if (lsb) {
			    ...
			    int len)
{
	...
	if (tx && rx) {
		while (len) {
			...
			memcpy(rx, ch34x_dev->bulkin_buf + USB20_CMD_HEADER,
			       bytes_to_copy);
			rx += bytes_to_copy;
			len -= bytes_to_copy;
			result = 0;
				  ...
				  struct spi_transfer *t)
{
	...
	if (ch34x_dev->chiptype == CHIP_CH341) {
		if (spi->mode & SPI_MODE_3) {
			/* use slow bitbang implementation for SPI_MODE_1,
			 * SPI_MODE_2 and SPI_MODE_3
			 */
			result = ch341_spi_bitbang(ch34x_dev, spi, t->tx_buf,
						   t->rx_buf, t->len);
		} else {
			/* otherwise the faster hardware implementation */
			result = ch341_spi_native(ch34x_dev, spi, t->tx_buf,
						  t->rx_buf, t->len);
		}
	} else {
		result = ch347_spi_native(ch34x_dev, spi, t->tx_buf, t->rx_buf,
					  t->len);
	}
...
static int ch341_spi_transfer_one_message(struct spi_master *ctlr,
					  struct spi_message *msg)
{
	...
	if (ch34x_dev->last_cpol != cpol) {
		...
		ch34x_dev->gpio_io_data =
		    (ch34x_dev->gpio_io_data & ~SCK_BIT) | (cpol ? SCK_BIT : 0);
		ch341_spi_update_io_data(ch34x_dev);
	}
...
static int ch347_spi_transfer_one_message(struct spi_master *ctlr,
					  struct spi_message *msg)
{
	struct ch34x_device *ch34x_dev = ch34x_spi_maser_to_dev(ctlr);
	struct spi_transfer *xfer;
	...
	if (((ch34x_dev->firmver >= 0x0341) ||
		...
		list_for_each_entry(xfer, &msg->transfers, transfer_list)
		{
			if ((xfer->tx_buf || xfer->rx_buf) && xfer->len) {
				...
				bytes_to_copy = ch347_spi_build_packet(
		...
		if (spi_device_get_chip_select(msg->spi, 0) == 0) {
			ch34x_dev->bulkout_buf[1] =
			    (u8)(bytes_to_xfer - USB20_CMD_HEADER);
			ch34x_dev->bulkout_buf[2] =
			    (u8)((bytes_to_xfer - USB20_CMD_HEADER) >> 8) |
			    BIT(7) | BIT(6);
		} else {
			ch34x_dev->bulkout_buf[1] =
			    (u8)(bytes_to_xfer - USB20_CMD_HEADER);
		...
		/* fill input data with input buffer */
		memcpy(rbuf, ch34x_dev->bulkin_buf + USB20_CMD_HEADER,
		       bytes_to_xfer - USB20_CMD_HEADER);
		...
		list_for_each_entry(xfer, &msg->transfers, transfer_list)
		{
			if ((xfer->tx_buf || xfer->rx_buf) && xfer->len) {
				reinit_completion(&ctlr->xfer_completion);
				memcpy(xfer->rx_buf, rbuf + msg->actual_length,
				       xfer->len);
			} else {
				if (xfer->len)
					dev_err(&msg->spi->dev,
		...
		list_for_each_entry(xfer, &msg->transfers, transfer_list)
		{
			if ((xfer->tx_buf || xfer->rx_buf) && xfer->len) {
				reinit_completion(&ctlr->xfer_completion);
				ret = ch34x_spi_transfer_one(ch34x_dev,
							     msg->spi, xfer);
				if (ret < 0) {
					dev_err(&msg->spi->dev,
						"SPI transfer failed: %d\n",
...
static int ch341_spi_setup(struct spi_device *spi)
{
	struct ch34x_device *ch34x_dev =
	    ch34x_spi_maser_to_dev(spi_device_get_master(spi));
	u8 cs_mask = (1 << spi_device_get_chip_select(spi, 0));
...
{
	struct spi_master *master = spi_device_get_master(spi);
	struct ch34x_device *ch34x_dev =
	    ch34x_spi_maser_to_dev(spi_device_get_master(spi));
	struct ch34x_spi_config spicfg = {0};
	u8 scale;
	int ret;
	int i, j;
	u8 clock_index;
	int clk_table0[] = {
	    60e6,  48e6,  36e6,	  30e6,	   28e6,   24e6,  18e6,	   15e6,
	    14e6,  12e6,  9e6,	  75e5,	   7e6,	   6e6,	  45e5,	   375e4,
	    35e5,  3e6,	  225e4,  1875e3,  175e4,  15e5,  1125e3,  9375e2,
	    875e3, 750e3, 5625e2, 46875e1, 4375e2, 375e3, 28125e1, 21875e1};
	int clk_table1[] = {
	...
	if (spi_device_get_chip_select(spi, 0) == 1) {
	...
	if (spi_device_get_chip_select(spi, 0) == 0)
		spicfg.cs = 0x80;
	else if (spi_device_get_chip_select(spi, 0) == 1)
		spicfg.cs = 0x80 << 8;
	else {
		ret = -EINVAL;
		goto exit;
	...
	dev_dbg(CH34X_USBDEV,
		"spimode:%d, max_speed_hz: %d, scale: %d, clock: %d\n",
		spicfg.mode, spi->max_speed_hz, scale, spicfg.clock);
...
int ch34x_spi_probe(struct ch34x_device *ch34x_dev)
{
	...
	/* allocate a new SPI master with a pointer to ch34x_device as device
	 * data */
	ch34x_dev->master =
	    spi_alloc_master(CH34X_USBDEV, sizeof(struct ch34x_device *));
	if (!ch34x_dev->master) {
	...
	/* register spi master */
	result = spi_register_master(ch34x_dev->master);
	if (result) {
		dev_err(CH34X_USBDEV, "could not register SPI master");
		spi_master_put(ch34x_dev->master);
		ch34x_dev->master = NULL;
		return result;
	}

	dev_info(CH34X_USBDEV, "SPI master connected to SPI bus %d",
	...
	for (i = 0; i < ch34x_dev->slave_num; i++) {
		...
		ch34x_dev->slaves[i] =
		    spi_new_device(ch34x_dev->master, &ch34x_spi_devices[i]);
		if (ch34x_dev->slaves[i]) {
			dev_info(CH34X_USBDEV,
				 "SPI device /dev/spidev%d.%d created",
				 ch34x_dev->master->bus_num,
