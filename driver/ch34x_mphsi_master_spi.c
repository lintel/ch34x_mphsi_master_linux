...
static void ch341_spi_update_io_data(struct ch34x_device *ch34x_dev) {
  ...
  ch34x_dev->bulkout_buf[2] =
      CH341_CMD_UIO_STM_OUT | (ch34x_dev->gpio_io_data & ch34x_dev->gpio_mask);
...
static void ch341_spi_set_cs(struct spi_device *spi, bool active) {
  ...
  if (spi_device_get_chip_select(spi, 0) > CH341_SPI_MAX_NUM_DEVICES) {
    DEV_ERR(CH34X_USBDEV, "invalid CS value %d, 0~%d are available",
            spi_device_get_chip_select(spi, 0), CH341_SPI_MAX_NUM_DEVICES - 1);
  }

  if (active)
    ch34x_dev->gpio_io_data &= ~(1 << spi_device_get_chip_select(spi, 0));
  else
    ch34x_dev->gpio_io_data |= (1 << spi_device_get_chip_select(spi, 0));
 ...
 */
static bool ch347spi_init(struct ch34x_device *ch34x_dev, mspi_cfgs *spicfg) {
  u8 *io = ch34x_dev->bulkout_buf;
...
static bool ch347spi_change_cs(struct ch34x_device *ch34x_dev, u8 istatus) {
  u16 ienable_cs; /* low 8 bits: CS0, high 8 bits: CS1, byte value -> 1: set CS,
                     0: ignore CS setting */
  u16 ics; /* low 8 bits: CS0, high 8 bits: CS1, CS output, byte value -> 1: set
              CS, 0: cancel CS */
  int iauto_de_cs;   /* low 16 bits: CS0, high 16 bits: CS1, automatically undo
                        the CS after operation completed */
  int iactive_delay; /* low 16 bits: CS0, high 16 bits: CS1, delay time of read
                        and write operation after setting CS, unit: us */
  int ideactive_delay; /* low 16 bits: CS0, high 16 bits: CS1,, delay time of
                          read and write operation after canceling CS, unit: us
                        */
  ...
  return ch347spi_set_cs(ch34x_dev, ienable_cs, ics, iauto_de_cs, iactive_delay,
                         ideactive_delay);
}
...
static int ch347_spi_native(struct ch34x_device *ch34x_dev,
  ...
  if (tx && rx) {
    while (len) {
      ...
      result =
          ch34x_usb_transfer(ch34x_dev, bytes_to_copy + USB20_CMD_HEADER, 0);
      ...
      result =
          ch34x_usb_transfer(ch34x_dev, 0, bytes_to_copy + USB20_CMD_HEADER);
      if ((result != (bytes_to_copy + USB20_CMD_HEADER)) ||
...
static int ch34x_spi_transfer_one(struct ch34x_device *ch34x_dev,
  ...
  if (ch34x_dev->chiptype == CHIP_CH341) {
    if (spi->mode & SPI_MODE_3) {
      /* use slow bitbang implementation for SPI_MODE_1, SPI_MODE_2 and
       * SPI_MODE_3 */
...
static int ch347_spi_transfer_one_message(struct spi_master *ctlr,
  ...
  if (((ch34x_dev->firmver >= 0x0341) ||
    ...
    if (spi_device_get_chip_select(msg->spi, 0) == 0) {
      ...
      ch34x_dev->bulkout_buf[2] =
          (u8)((bytes_to_xfer - USB20_CMD_HEADER) >> 8) | BIT(7) | BIT(6);
    } else {
      ch34x_dev->bulkout_buf[1] = (u8)(bytes_to_xfer - USB20_CMD_HEADER);
      ch34x_dev->bulkout_buf[2] =
          (u8)((bytes_to_xfer - USB20_CMD_HEADER) >> 8) | BIT(5) | BIT(4);
    }
...
int ch34x_spi_probe(struct ch34x_device *ch34x_dev) {
  ...
  for (i = 0; i < ch34x_dev->slave_num; i++) {
    ...
    if ((ch34x_dev->slaves[i] =
      ...
      DEV_INFO(CH34X_USBDEV, "SPI device /dev/spidev%d.%d created",
               ch34x_dev->master->bus_num, ch34x_spi_devices[i].chip_select);
    }
  }
#endif
